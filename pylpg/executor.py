...
def exec_lpg(filename):
    ...
    for x in data["Assigns"]:
        ...
        for person in lpgPersons:
            pd = lpgdata.PersonLivingTag(
                person["LivingPatternTag"], person["PersonName"]
            )
        ...
        hd = lpgdata.HouseholdData(
            HouseholdTemplateSpec=templatespec,
            UniqueHouseholdId="HH" + str(i),
        ...
        hd.Name = "HH" + str(i)
        hd.HouseholdDataSpecification = (
            lpgpythonbindings.HouseholdDataSpecificationType.ByTemplateName
        )
