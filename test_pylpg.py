...
def test_single_household_function() -> None:
    random.seed(2)
    df: pandas.DataFrame = pylpg.excute_lpg_single_household(year=2020, householdref=Households.CHR04_Couple_30_64_years_1_at_work_1_at_home,
...
def test_many_householdata_function() -> None:
        ...
        hhdata2 = HouseholdData(UniqueHouseholdId="uniq", Name="name",
                                HouseholdDataPersonSpec=personspec,
                                HouseholdDataSpecification=HouseholdDataSpecificationType.ByPersons,
                                TransportationDeviceSet=TransportationDeviceSets.Bus_and_two_30_km_h_Cars,
                                ChargingStationSet=ChargingStationSets.Charging_At_Home_with_11_kW,
        ...
        df: pandas.DataFrame = pylpg.excute_lpg_with_many_householdata(year=2020, householddata=[hhdata1, hhdata2],
                                                                  housetype=HouseTypes.HT23_No_Infrastructure_at_all,
                                                                  startdate="01.01.2020", enddate="01.03.2020",
                                                                  simulate_transportation=True)
        df.to_csv(r'lpgexport.csv', index=True, sep=";")
...
def test_householdata_function_with_livingpatterns() -> None:
    random.seed(2)
    p1 = PersonLivingTag(LivingPatternTag=LivingPatternTags.Living_Pattern_Work_From_Home_Full_Time_5_days, PersonName=TemplatePersons.CHR01_1_25M.PersonName)
    p2 = PersonLivingTag(LivingPatternTag=LivingPatternTags.Living_Pattern_Work_From_Home_Full_Time_5_days,
                         PersonName=TemplatePersons.CHR01_0_23F.PersonName)
    personspec = HouseholdTemplateSpecification([p1, p2], HouseholdTemplateName=HouseholdTemplates.CHR01_Couple_both_at_Work, ForbiddenTraitTags=[
        TraitTags.Food_Brunching
    ])
    hhdata1 = HouseholdData(UniqueHouseholdId="uniq", Name="name",
    ...
    df: pandas.DataFrame = pylpg.excute_lpg_with_householdata(year=2020, householddata=hhdata1,
                                                              housetype=HouseTypes.HT23_No_Infrastructure_at_all,
                                                              startdate="01.01.2020", enddate="2020-01-31"
                                                              )
    print(df)
...
def test_family_maker() -> None:
    persons1: List[PersonData] = pylpg.make_reasonable_family(1)
    print_persons_list(persons1)
    persons2: List[PersonData] = pylpg.make_reasonable_family(2)
    print_persons_list(persons2)
    persons3: List[PersonData] = pylpg.make_reasonable_family(3)
    print_persons_list(persons3)
    persons4: List[PersonData] = pylpg.make_reasonable_family(4)
    print_persons_list(persons4)
    persons5: List[PersonData] = pylpg.make_reasonable_family(5)
...
if __name__ == "__main__":
    ...
    test_many_householdata_function()
